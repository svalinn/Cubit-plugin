                                  ...
                                  const Tag* /*file_id_tag*/)
{
  ...
  ErrorCode result = process_options(opts);CHK_SET_ERR_STR(result, fileName << ": problem reading options");
                                      ...
                                      EntityHandle &start_elem, Range &read_ents)
{
  ...
  ErrorCode result = readMeshIface->get_element_connect(num_elems, verts_per_elem, ent_type,
                                                        1, start_elem, conn_array);CHK_SET_ERR_STR(result, fileName << ": Trouble reading elements");
                                    ...
                                    EntityHandle /*start_elem*/, int /*num_elems*/, Range &read_ents)
{
  ...
  for (int i = 0; i < num_sets; i++) {
    // Create set
    result = mbImpl->create_meshset(MESHSET_SET, this_set);CHK_SET_ERR_STR(result, fileName << ": Trouble creating set");

    Range set_ents;
    // Read/compute what's in this set; REMEMBER TO CONVERT THESE TO MOAB HANDLES

    // Add them to the set
    result = mbImpl->add_entities(this_set, set_ents);CHK_SET_ERR_STR(result, fileName << ": Trouble putting entities in set");
