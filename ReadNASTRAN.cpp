                                 ...
                                 const Tag* file_id_tag)
{
  ...
  bool debug = false;
  if (debug)
    std::cout << "begin ReadNASTRAN::load_file" << std::endl;
  ErrorCode result;
  ...
  if (MB_SUCCESS != result)
    return result;
  if (0 == start_vert)
    return MB_FAILURE; // check for NULL
  int id, vert_index = 0;
  if (debug)
    std::cout << "allocated coord arrays" << std::endl;
...
ErrorCode ReadNASTRAN::get_real(const std::string &token, double &real)
{
  ...
  if (HUGE_VAL == signi || HUGE_VAL == expon)
    return MB_FAILURE;
                                 ...
                                 int& id)
{
  ...
  for (unsigned int i = 0; i < 3; i++) {
    ...
    if (MB_SUCCESS != result)
      return result;
    if (debug)
      std::cout << "read_node: coords[" << i << "]=" << coords[i] << std::endl;
  }
