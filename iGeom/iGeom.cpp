...
//  if (NULL == dynamic_cast<Body*>(this_ent)) return;
  ...
  if (NULL != this_vol || NULL != this_body) {
      // need to get the associated body, since cgm only supports copying
      // bodies, not volumes
    if (NULL == this_body) {
                       ...
                       int* err)
{
  ...
  if (0.0 >= tmp_x || 0.0 >= tmp_y || 0.0 >= tmp_z) {
    ...
    error_message << "Dimensions must be >= 0, or y & z must both be zero."
                  << std::endl;
    CubitMessage::get_message_handler()->print_error(error_message.str().c_str());
  }
  else {

  RefEntity *temp_body = GeometryModifyTool::instance()->brick(tmp_x,
                                                               tmp_y,
                                                               tmp_z);
                      ...
                      int* err)
{
  ...
  RefEntity *temp_body =
    GeometryModifyTool::instance()->cylinder(height, major_rad_base,
                                             tmp_minor, rad_top);
  *geom_entity = reinterpret_cast<iBase_EntityHandle>(temp_body);
                          ...
                          int* err)
{
  ...
  RefEntity *temp_body =
    GeometryModifyTool::instance()->cylinder(height, major_rad,
                                             tmp_minor, major_rad);
  *geom_entity = reinterpret_cast<iBase_EntityHandle>(temp_body);
                       ...
                       int* err)
{
  if (minor_rad >= major_rad) {
    ...
    RefEdge *curve = GeometryModifyTool::instance()->make_RefEdge(
    ...
    CubitStatus status = GeometryModifyTool::instance()->sweep_rotational(
        surfs_to_sweep, sweep_point,
    ...
    *geom_entity = reinterpret_cast<iBase_EntityHandle>(temp_body);
    ...
    RefEntity *temp_body = GeometryModifyTool::instance()->torus(major_rad,
                                                                 minor_rad);
    *geom_entity = reinterpret_cast<iBase_EntityHandle>(temp_body);
  }
                       ...
                       int* err)
{
  if (RefGroup *this_set = reinterpret_cast<RefGroup*>(set_handle)) {
    ...
    iGeomArrayManager gentity_handles_manager(
        reinterpret_cast<void**>(gentity_handles),
        *(gentity_handles_allocated),
        *(gentity_handles_size),
        dim_entities.size(),
        sizeof(**gentity_handles),
        err);
...
void iGeom_imprintEnts(iGeom_Instance instance,
                       ...
                       int* err)
{
  // GMT::imprint segfaults if passed an empty list
                         ...
                         int* err)
{
  ...
  */
  ...
  CubitStatus result = GeometryModifyTool::instance()->intersect(ent2_copy,
                                                                 ent1_list,
                                                                 new_body_list);
  ...
  if (0 == new_body_list.size()) {
    CubitMessage::get_message_handler()->print_error("Intersect Failed");
    return;
  }
  else {
                      ...
                      int* err)
{
  ...
  ERROR(iBase_INVALID_ENTITY_TYPE,
        "Wrong type of entity specified for reflect.");
}
                    ...
                    int* err)
{
  ...
  CubitStatus result;
  if (NULL != this_ent) {
    GeometryQueryTool::instance()->scale(ents, point, scale_x, scale_y,
                                         scale_z, true, ents);
    if (CUBIT_SUCCESS != result) {
                      ...
                      int* err)
{
  ...
  CubitStatus result = GeometryModifyTool::instance()->section(blank_list,
                                                               point1,
                                                               point2,
                                                               point3,
                                                               new_body_list,
                   ...
                   int* err)
{
  ...
  iBase_ErrorType retval = reinterpret_cast<CGMTagManager*>(instance)->setArrData(
      &tmp_entity, 1, reinterpret_cast<long>(tag_handle), tag_value);
  RETURN(retval);
}
                        ...
                        int* err)
{
  ...
  RefEntity *new_body = NULL;
  CubitStatus result = GeometryModifyTool::instance()->subtract(tool_copy,
                                                                blank_list,
                                                                new_body_list);
                     ...
                     int* err)
{
  ...
  CubitStatus result = GeometryModifyTool::instance()->unite(bods, new_bods,
                                                             false);
                ...
                int options_len)
{
  ...
  if (file_type.empty()) {
    ...
    if (name_buf.find(".igs") < name_len ||
      ...
      ERROR(iBase_FAILURE,
            "Unknown geometry file extension and no file type.");
  }

  /**
   * Work around AcisQueryEngine log file handling bug: If the export format
   * is iges or step, be sure the log file name is not null
   */
  ...
  CubitStatus status = CubitCompat_export_solid_model(bodies,
                                                      name,
                                                      file_type.c_str(),
                                                      num_ents_exported,
                                                      cubit_version,
                                      ...
                                      CubitVector& maxc)
{
  ...
  else if (Body* body = dynamic_cast<Body*>(entity))
  {
    ...
    CGM_iGeom_setLastError(iBase_INVALID_ENTITY_HANDLE,
                           "Entities passed into gentityBoundingBox must be vertex, edge, face, or region.");
                     ...
                     int* err)
{
  ...
  switch (ibase_type) {
    case iBase_ALL_TYPES:
      count = append_not_type<RefGroup>(list, *entity_handles,
                                        *entity_handles_alloc);
      break;
    case iBase_REGION:
      count = append_type<Body>(list, *entity_handles, *entity_handles_alloc);
      break;
    case iBase_FACE:
      count = append_type<RefFace>(list, *entity_handles,
                                   *entity_handles_alloc);
      break;
    case iBase_EDGE:
      count = append_type<RefEdge>(list, *entity_handles,
                                   *entity_handles_alloc);
      break;
    case iBase_VERTEX:
      count = append_type<RefVertex>(list, *entity_handles,
                                     *entity_handles_alloc);
      break;
    default:
...
static inline
int count_ibase_type(int ibase_type, const DLIList<CubitEntity*>& list,
                     int* err)
{
  *err = iBase_SUCCESS;
  switch (ibase_type) {
...
CubitStatus CubitCompat_export_solid_model(
    DLIList<RefEntity*>& ref_entity_list, const char* filename,
    const char * filetype, int &num_ents_exported,
    const CubitString &cubit_version, const char* logfile_name)
{
    ...
    : interfaceGroup(NULL)
{
  ...
  if (CUBIT_FAILURE == status) {
    CGM_iGeom_setLastError(iBase_FAILURE,
                           "Couldn't create cgm attribute for tags.");
  }
  else
  ...
  if (tag_handle < 0) {
    for (int i = 0; i < entity_handles_size; i++) {
      if (NULL == entity_handles[i])
        tmp_result = setPresetTagData(interface_group(), tag_handle,
                                      val_ptr, tag_size);
      else
        tmp_result = setPresetTagData(entity_handles[i], tag_handle,
                                      val_ptr, tag_size);
...
//      RETURN(iBase_SUCCESS);
    ...
    case 2:
        // entity id
      CGM_iGeom_setLastError(iBase_NOT_SUPPORTED,
                             "Can't set id of entities with this implementation.");
      return iBase_NOT_SUPPORTED;
    case 3:
        // unique id
      CGM_iGeom_setLastError(iBase_NOT_SUPPORTED,
                             "Can't set unique id of entities with this implementation.");
      return iBase_NOT_SUPPORTED;
    ...
    default:
      CGM_iGeom_setLastError(iBase_NOT_SUPPORTED,
                             "Can't set this tag on entities with this implementation.");
      return iBase_NOT_SUPPORTED;
  }
...
CATag *CGMTagManager::get_catag(RefEntity *ent, const bool create_if_missing)
{
  CubitAttrib *this_attrib = ent->get_cubit_attrib(CATag_att_type,
                                                   create_if_missing);
  if (NULL != this_attrib)
...
CubitSimpleAttrib CATag::cubit_simple_attrib()
{
  ...
  CubitSimpleAttrib *csa = new CubitSimpleAttrib(&str_data, &dbl_data,
                                                 &int_data);
...
CATag::CATag(CGMTagManager *manager, RefEntity *owner,
