...
const char* FileTokenizer::get_string()
{
  ...
  for (;;) {
    ...
    if (nextToken == bufferEnd) {
      ...
      if (0 == count) {
        if (feof(filePtr))
          SET_ERR_STR_RET_VAL("File truncated at line " << line_number(), NULL);
        else
...
bool FileTokenizer::get_double_internal(double& result)
{
  ...
  if (token[0] && token[1] && token[0] == '0' && toupper(token[1]) == 'X')
    SET_ERR_STR_RET_VAL("Syntax error at line " << line_number() << ": expected number, got \"" << token << "\"", false);
...
bool FileTokenizer::get_byte_internal(unsigned char& result)
{
  ...
  if (i != (long)result)
    SET_ERR_STR_RET_VAL("Numeric overflow at line " << line_number(), false);
...
bool FileTokenizer::get_boolean_internal(bool& result)
{
  ...
  if (token[1] || (token[0] != '0' && token[0] != '1'))
    SET_ERR_STR_RET_VAL("Syntax error at line " << line_number() << ": expected 0 or 1, got \"" << token << "\"", false);
...
bool FileTokenizer::match_token(const char* str, bool print_error)
{
  ...
  if (print_error)
    SET_ERR_STR_CONT("Syntax error at line " << line_number() << ": expected \"" << str << "\", got \"" << token << "\"");
...
bool FileTokenizer::get_newline()
{
  ...
  for (;;) {
    ...
    if (nextToken == bufferEnd) {
      ...
      if (0 == count) {
        if (eof())
          SET_ERR_STR_RET_VAL("File truncated at line " << line_number(), false);
        else
    ...
    if (!isspace(*nextToken))
      SET_ERR_STR_RET_VAL("Expected newline at line " << line_number(), false);
