...
#define GET_DIM(ncdim, name, val) \
  { \
    ...
    if (NC_NOERR == gdfail) { \
      ...
      if (NC_NOERR != gdfail) { \
        SET_ERR(MB_FAILURE, "WriteNCDF:: couldn't get dimension length"); \
      } \
      else \
        val = tmp_val; \
    } \
                                             ...
                                             std::vector<EntityHandle> &nodesets)
{
  ...
  for (vector_iter = blocks.begin(); vector_iter != blocks.end(); ++vector_iter) {
    ...
    if (mdbImpl->tag_get_data(mMaterialSetTag, &(*vector_iter), 1, &id) != MB_SUCCESS) {
      SET_ERR(MB_FAILURE, "Couldn't get block id from a tag for an element block");
    }
  ...
  for ( ; vector_iter != end_vector_iter; ++vector_iter) {
    ...
    for ( ; iter != end_iter; ++iter) {
      if (TYPE_FROM_HANDLE(*iter) != MBVERTEX)
        continue;
      result = mdbImpl->tag_get_data(mEntityMark, &(*iter), 1, &node_marked);CHK_SET_ERR(result, "Couldn't get mark data");
  ...
  for ( ; vector_iter != end_vector_iter; ++vector_iter) {
    ...
    ErrorCode result = get_valid_sides(forward_elems, mesh_info, 1, sideset_data);CHK_SET_ERR(result, "Couldn't get valid sides data");
    result = get_valid_sides(reverse_elems, mesh_info, -1, sideset_data);CHK_SET_ERR(result, "Couldn't get valid sides data");
                                     ...
                                     NeumannSetData &sideset_data)
{
  ...
  for (Range::iterator iter = elems.begin(); iter != elems.end(); ++iter) {
    ...
    result = mdbImpl->tag_get_data(mEntityMark, &(*iter), 1, &element_marked);CHK_SET_ERR(result, "Couldn't get mark data");
    ...
    if (0x1 == element_marked) {
      ...
      if ( mdbImpl->get_adjacencies(&(*iter), 1, dimension + 1, false, parents) != MB_SUCCESS) {
        SET_ERR_CONT("Warning: Couldn't get adjacencies for sideset");
      }

      if (!parents.empty()) {
        // Make sure the adjacent parent element will be output
        for (unsigned int k = 0; k < parents.size(); k++) {
          result = mdbImpl->tag_get_data(mEntityMark, &(parents[k]), 1, &element_marked);CHK_SET_ERR(result, "Couldn't get mark data");
                                     ...
                                     int record_position)
{
  ...
  if (-1 == temp_var) {
    SET_ERR(MB_FAILURE, "WriteNCDF:: Problem getting qa record variable");
  }
  ...
  if (NC_NOERR != fail) {
    SET_ERR(MB_FAILURE, "Failed to position qa string variable");
  }
...
ErrorCode WriteNCDF::write_nodes(int num_nodes, Range& nodes, int dimension)
{
  ...
  if (-1 == nc_var) {
    SET_ERR(MB_FAILURE, "Trouble getting coordinate name variable");
  }
  ...
  if (transform_needed) {
    ...
    result = mdbImpl->tag_get_data(trans_tag, &mesh, 0, trans_matrix);CHK_SET_ERR(result, "Couldn't get transform data");
...
ErrorCode WriteNCDF::write_elementblocks(std::vector<MaterialSetData> &block_data)
{
  ...
  for (i = 0; i < block_data.size(); i++) {
    ...
    if (write_exodus_integer_variable("eb_status", &status, block_index, num_values) != MB_SUCCESS) {
      SET_ERR(MB_FAILURE, "Problem writing element block status");
    }
    ...
    if (result != MB_SUCCESS) {
      delete [] connectivity;
      SET_ERR(result, "Couldn't get element array to write from");
    }
    ...
    if (-1 == nc_var) {
      delete [] connectivity;
      SET_ERR(MB_FAILURE, "Couldn't get connectivity variable");
    }
    ...
    if (NC_NOERR != fail) {
      delete [] connectivity;
      SET_ERR(MB_FAILURE, "Couldn't write connectivity variable");
    }
...
ErrorCode WriteNCDF::write_global_node_order_map(int num_nodes, Range& nodes)
{
  ...
  if (error < 0) {
    SET_ERR(MB_FAILURE, "Failed writing global node order map");
  }
...
ErrorCode WriteNCDF::write_global_element_order_map(int num_elements)
{
  ...
  if (error < 0) {
    SET_ERR(MB_FAILURE, "Failed writing global element order map");
  }
...
ErrorCode WriteNCDF::write_element_order_map(int num_elements)
{
  ...
  if (error < 0) {
    SET_ERR(MB_FAILURE, "Failed writing element map");
  }
                               ...
                               std::vector<DirichletSetData> &nodesets)
{
  ...
  for (std::vector<DirichletSetData>::iterator ns_it = nodesets.begin();
    ...
    for ( ; begin_iter != end_iter; ++begin_iter) {
      result = mdbImpl->tag_get_data(mGlobalIdTag, &(*begin_iter), 1, &exodus_id);CHK_SET_ERR(result, "Problem getting id tag data");
    ...
    result = write_exodus_integer_variable("ns_status",
                                           &status, ns_index, num_values);CHK_SET_ERR_RET_VAL(result, "Problem writing node set status", MB_FAILURE);
    ...
    GET_VAR(wname, nc_var, dims);
    if (-1 == nc_var) {
      SET_ERR(MB_FAILURE, "Failed to get dist_fact variable");
    }
    fail = nc_put_vara_double(ncFile, nc_var, &start, &count, dist_factor_array);
    if (NC_NOERR != fail) {
      SET_ERR(MB_FAILURE, "Failed writing dist factor array");
    }
  ...
  for (i = 0; i < sidesets.size(); i++) {
    ...
    if (0 != number_elements) {
      ...
      result = write_exodus_integer_variable("ss_status",
                                             &status,
                                             ss_index, num_values);CHK_SET_ERR_RET_VAL(result, "Problem writing side set status", MB_FAILURE);
      ...
      GET_VAR(wname, nc_var, dims);
      if (-1 == nc_var) {
        SET_ERR(MB_FAILURE, "Failed to get side_ss variable");
      }
      fail = nc_put_vara_int(ncFile, nc_var, &start, &count, output_element_side_numbers);
      if (NC_NOERR != fail) {
        SET_ERR(MB_FAILURE, "Failed writing sideset side array");
      }
      ...
      GET_VAR(wname, nc_var, dims);
      if (-1 == nc_var) {
        SET_ERR(MB_FAILURE, "Failed to get sideset dist factors variable");
      }
      count = sideset_data.ss_dist_factors.size();
      fail = nc_put_vara_double(ncFile, nc_var, &start, &count, &(sideset_data.ss_dist_factors[0]));
      if (NC_NOERR != fail) {
        SET_ERR(MB_FAILURE, "Failed writing sideset dist factors array");
      }
    }
                                            ...
                                            bool /* write_sideset_distribution_factors */)
{
  ...
  if (nc_def_dim(ncFile, "len_string", ExoIIInterface::MAX_STR_LENGTH, &dim_str) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to get string length in file");
  }

  if (nc_def_dim(ncFile, "len_line", ExoIIInterface::MAX_STR_LENGTH, &dim_line) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to get line length in file");
  }

  if (nc_def_dim(ncFile, "four", 4, &dim_four) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to locate four in file");
  }

  if (nc_def_dim(ncFile, "time_step", 1, &dim_time) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to locate time step in file");
  }
  ...
  if (NC_NOERR != nc_put_att_text(ncFile, NC_GLOBAL, "title", length, working_title)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define title attribute");
  }
  ...
  if (NC_NOERR != nc_put_att_float(ncFile, NC_GLOBAL, "api_version", NC_FLOAT, 1, &dum_vers)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define api_version attribute");
  }
  dum_vers = 2.05F;
  if (NC_NOERR != nc_put_att_float(ncFile, NC_GLOBAL, "version", NC_FLOAT, 1, &dum_vers)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define version attribute");
  }
  int dum_siz = sizeof(double);
  if (NC_NOERR != nc_put_att_int(ncFile, NC_GLOBAL, "floating_point_word_size", NC_INT, 1, &dum_siz)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define floating pt word size attribute");
  }
  ...
  if (nc_def_dim(ncFile, "num_dim", (size_t)mesh_info.num_dim, &num_dim) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of dimensions");
  }

  if (nc_def_dim(ncFile, "num_nodes", mesh_info.num_nodes, &num_nodes) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of nodes");
  }

  if (nc_def_dim(ncFile, "num_elem", mesh_info.num_elements, &num_elem) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of elements");
   }

  if (nc_def_dim(ncFile, "num_el_blk", mesh_info.num_elementblocks, &num_el_blk) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of element blocks");
  }
  ...
  if (NC_NOERR != nc_def_var(ncFile, "eb_status", NC_LONG, 1, &num_el_blk, &idstat)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define element block status array");
  }
  ...
  if (NC_NOERR != nc_def_var(ncFile, "eb_prop1", NC_LONG, 1, &num_el_blk, &idarr)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define element block id array");
  }

  /*   store property name as attribute of property array variable */
  if (NC_NOERR != nc_put_att_text(ncFile, idarr, "name", strlen("ID"), "ID")) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to store element block property name ID");
  }
  ...
  if (non_empty_nss > 0) {
    if (nc_def_dim(ncFile, "num_node_sets", (size_t)(non_empty_nss), &num_ns) != NC_NOERR) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of node sets");
    }

    /* Node set id status array: */

    if (NC_NOERR != nc_def_var(ncFile, "ns_status", NC_LONG, 1, &num_ns, &ns_idstat)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to create node sets status array");
    }

    /* Node set id array: */
    if (NC_NOERR != nc_def_var(ncFile, "ns_prop1", NC_LONG, 1, &num_ns, &ns_idarr)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to create node sets property array");
    }

    /* Store property name as attribute of property array variable */
    if (NC_NOERR != nc_put_att_text(ncFile, NC_GLOBAL, "name", strlen("ID"), "ID")) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to store node set property name ID");
    }
  ...
  if (non_empty_ss > 0) {
    ...
    if (nc_def_dim(ncFile, "num_side_sets", non_empty_ss, &num_ss) != NC_NOERR) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to define number of side sets");
    }

    /* Side set id status array: */
    int ss_idstat = -1, ss_idarr = -1;
    if (NC_NOERR != nc_def_var(ncFile, "ss_status", NC_LONG, 1, &num_ss, &ss_idstat)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to define side set status");
    }

    /* Side set id array: */
    if (NC_NOERR != nc_def_var(ncFile, "ss_prop1", NC_LONG, 1, &num_ss, &ss_idarr)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to define side set property");
    }

    /* Store property name as attribute of property array variable */
    if (NC_NOERR != nc_put_att_text(ncFile, ss_idarr, "name", strlen("ID"), "ID")) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to store side set property name ID");
    }
  ...
  if (NC_NOERR != nc_def_var(ncFile, "coord", NC_DOUBLE, 2, dims, &coord)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define node coordinate array");
  }
  ...
  if (NC_NOERR != nc_def_var(ncFile, "coor_names", NC_CHAR, 2, dims, &name_coord)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define coordinate name array");
  }
  ...
  if (write_maps) {
    ...
    if (NC_NOERR != nc_def_var(ncFile, "elem_map", NC_LONG, 1, &num_elem, &elem_map)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to create element map array"); /* Exit define mode and return */
    }

    // Create the element number map
    if (NC_NOERR != nc_def_var(ncFile, "elem_num_map", NC_LONG, 1, &num_elem, &elem_map2)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to create element numbering map"); /* Exit define mode and return */
    }

    // Create node number map
    if (NC_NOERR != nc_def_var(ncFile, "node_num_map", NC_LONG, 1, &num_nodes, &node_map)) {
      SET_ERR(MB_FAILURE, "WriteNCDF: failed to create node numbering map array"); /* Exit define mode and return */
    }
  }
  ...
  if (nc_def_dim(ncFile, "num_qa_rec", (long)num_qa_rec, &num_qa) != NC_NOERR) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define qa record array size");
  }
  ...
  if (NC_NOERR != nc_def_var(ncFile, "qa_records", NC_CHAR, 3, dims, &qa_title)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: failed to define qa record array");
  }

  // Take it out of define mode
  if (NC_NOERR != nc_enddef(ncFile)) {
    SET_ERR(MB_FAILURE, "WriteNCDF: Trouble leaving define mode");
  }
...
ErrorCode WriteNCDF::open_file(const char* filename)
{
  ...
  if (strlen((const char*)filename) == 0) {
    SET_ERR(MB_FAILURE, "Output Exodus filename not specified");
  }
